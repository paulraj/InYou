...
+ (instancetype)sharedManager {
    static AWSContentManager *_sharedManager = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _sharedManager = [[AWSContentManager alloc] initWithType:AWSContentManagerTypeS3
                                                          bucket:AWS_CONTENT_MANAGER_S3_BUCKET_NAME
                                                          domain:nil
                                                      identifier:AWS_CONTENT_MANAGER_S3_BUCKET_NAME];
    });
...
- (void)GETURLForKey:(NSString *)key
   completionHandler:(void (^)(NSURL *, NSError *))completionHandler {
    NSURL *URL = [NSURL URLWithString:key
                        relativeToURL:[NSURL URLWithString:[NSString stringWithFormat:@"https://%@",self.domain]]];
    completionHandler(URL, nil);
}
