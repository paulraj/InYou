...
class ContactsViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, UISearchBarDelegate, UISearchDisplayDelegate {
        ...
        if let hasContactsImageLoaded = userDefaults.objectForKey("contactsImageLoaded") as? Bool {
            if hasContactsImageLoaded == true {
                ...
                case "Twitter":
                    ...
                    TwitterAPIHandler().getTwitterContactImage(twitterContacts){ data, error in
                        //print("loadContactImage is complete.")
                        //self.filterContacts()
                        //GoogleContactsAPIHandler().sortContacts()
                        loadingContacts.hide()
    ...
    func tableView(tableView: UITableView, willDisplayCell cell: UITableViewCell, forRowAtIndexPath indexPath: NSIndexPath) {
        ...
        case "Twitter" :
            if indexPath.row == 24*pageNumber {
                loadingContacts.show()
                TwitterAPIHandler().getFriendslistWithUserId(loggedInUser.twitterFriendsListCursor) { data, error in
                    self.tableContacts = twitterContacts
                    TwitterAPIHandler().getTwitterContactImage(twitterContacts, pageNumber: self.pageNumber){ data, error in
                        //print("loadContactImage is complete for twitter.")
                        //self.filterContacts()
                        loadingContacts.hide()
                        self.tableViewObject.reloadData()
